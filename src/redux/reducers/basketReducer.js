...
export const basketReducer = (state = initialState, action) => {
  switch (action.type) {
    case basketConstants.ADD_PRODUCT:
      let itemExist = state.basket.some(
      ...
      if (itemExist) {
        ...
        const newProduct = Object.assign(action.payload, {
          totalPrice: Number(action.payload.unitPrice),
          amount: 1,
    ...
    case basketConstants.DELETE_PRODUCT:
      const foundProductIndex = state.basket.findIndex(
    ...
    case basketConstants.INCREMENT_PRODUCT:
      const existedIncrementProduct = state.basket.find(
        (item) => item.id === action.payload,
      );
      const existedIncrementProductIndex = state.basket.findIndex(
        (item) => item.id === action.payload,
      );
      ...
      newBasketWithIncrementProducts.splice(
        existedIncrementProductIndex,
        1,
        existedIncrementProduct,
      );
      return {
        ...state,
    ...
    case basketConstants.DECREMENT_PRODUCT:
      const existedDecrementProduct = state.basket.find(
        (item) => item.id === action.payload,
      );
      const existedDecrementProductIndex = state.basket.findIndex(
        (item) => item.id === action.payload,
      );
      ...
      newBasketWithDecrementProducts.splice(
        existedDecrementProductIndex,
        1,
        existedDecrementProduct,
      );
