...
export const getCuisineTypeValue = (categories) => {
  return categories.map((category) =>
    cuisineType.find((cuisine) => category === cuisine.key),
  );
};
  ...
  basket.forEach(function (a) {
    if (!this[a.restaurantId]) {
      this[a.restaurantId] = {
        ...
        paymentOnline: a.restaurantPaymentOnline,
        products: [],
      };
...
export const countMinTimeToPrepare = (basket) => {
  if (basket.length) {
    return Math.max.apply(
      Math,
      basket.map((product) => product.timeToDo),
    );
  } else {
    return '-';
  }
};
export const handleRenderMenuByCategory = (meals) =>
  meals.reduce(
    (acc, current) => ({
      ...acc,
